<x-app-layout :title="$alsintan->exists ? 'Ubah Data Alsintan' : (request()->routeIs('survey.create') ? 'Survei Lapangan' : 'Tambah Alsintan')">
    <section class="panel">
        ...
        <form method="post" enctype="multipart/form-data" action="{{ $alsintan->exists ? route('alsintans.update', $alsintan) : route('alsintans.store') }}">
            @csrf
            @if (request()->routeIs('survey.create')) <input type="hidden" name="survey" value="1"> @endif
            ...
            <div class="form-grid">
                ...
                <div class="field"><label>Foto survei</label><input type="file" name="photo" accept="image/*" capture="environment">@error('photo')<span class="error">{{ $message }}</span>@enderror</div>
