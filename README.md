...
Isi `SIMANTAP_ADMIN_EMAIL` dan `SIMANTAP_ADMIN_PASSWORD` pada `.env` sebelum menjalankan seeder. Seeder hanya membuat akun administrator jika belum ada, tidak membuat data contoh, dan tidak mengubah password akun yang sudah ada.

## Deployment produksi

1. Siapkan PHP 8.3+, ekstensi `pdo_mysql`, `mbstring`, `fileinfo`, `openssl`, `xml`, `curl`, dan `zip`, lalu buat database produksi.
2. Isi `.env` produksi dengan `APP_ENV=production`, `APP_DEBUG=false`, `APP_URL` HTTPS yang benar, `APP_KEY` hasil `php artisan key:generate`, `SESSION_SECURE_COOKIE=true`, kredensial database, `FILESYSTEM_DISK=public`, SMTP yang aktif, serta `SIMANTAP_ADMIN_EMAIL` dan `SIMANTAP_ADMIN_PASSWORD` yang kuat.
3. Setelah kode terbaru sudah berada di server, jalankan `bash deploy.sh` dari root aplikasi. Skrip ini memasang dependency produksi, menghapus cache lama, menjalankan migrasi, membangun aset Vite baru, lalu mengaktifkan kembali aplikasi.
   ...
   Skrip menjalankan seeder aman untuk memastikan akun admin tersedia. Seeder tidak membuat data dummy dan tidak menimpa akun maupun password yang sudah ada.
...
`public/build` tidak disimpan di repository. Karena itu `npm ci` dan `npm run build` wajib dijalankan pada setiap deploy agar server tidak memuat CSS/JavaScript versi lama. `php artisan optimize` meng-cache konfigurasi, event, route, dan view untuk produksi.
